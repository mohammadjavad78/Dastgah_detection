...
class Birch_Clustering :
    ...
    def printConfusionMatrix(self):
        for i in range (0,7):
            ypred = Birch(n_clusters=i+1).fit_predict(self.data)
    ...
    def printDataMetrics(self, arr, fReduction):
        ...
        for i in arr:
            if (fReduction):
                ypred = Birch(n_clusters=i).fit_predict(reduced_data)
                usedData = reduced_data
            else:
                ypred = Birch(n_clusters=i).fit_predict(self.data)
                usedData = self.data
    ...
    def plotClusters(self, nClusters, fReduction):
        ...
        for i in range(0,nClusters):
            ypred_reducedData = Birch(n_clusters=i+1).fit_predict(reduced_data)
            y_pred_data = Birch(n_clusters=i+1).fit_predict(self.data)
            if (fReduction):
    ...
    def plotClustersUsingArr(self, arr, fReduction):
        ...
        for i,j in zip(arr, range(len(arr))):
            ypred_reducedData = Birch(n_clusters=i).fit_predict(reduced_data)
            y_pred_data = Birch(n_clusters=i).fit_predict(self.data)
            if (fReduction):
                hueType = ypred_reducedData
                y_pred = ypred_reducedData
                figureName = ' cluster(s) (using UMAP feaure reduction method)'
    ...
    def elbow(self, rangeOfClusters):
        ...
        for k in K:
            BirchModel = Birch()
            BirchModel.fit(self.data)
            distortions.append(BirchModel.inertia_)
